...
void menu_temperature() {
  ...
  //
    ...
    #if HAS_FAN3
      editable.uint8 = thermalManager.fan_speed[3];
      EDIT_ITEM_FAST_N(percent, 4, MSG_FAN_SPEED_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(3, editable.uint8); });
      #if ENABLED(EXTRA_FAN_SPEED)
        EDIT_ITEM_FAST_N(percent, 4, MSG_EXTRA_FAN_SPEED_N, &thermalManager.new_fan_speed[3], 3, 255);
      #endif
    #elif ENABLED(SINGLENOZZLE) && EXTRUDERS > 3
      editable.uint8 = thermalManager.fan_speed[3];
      EDIT_ITEM_FAST_N(percent, 4, MSG_STORED_FAN_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(3, editable.uint8); });
    #endif

    #if HAS_FAN4
      editable.uint8 = thermalManager.fan_speed[4];
      EDIT_ITEM_FAST_N(percent, 5, MSG_FAN_SPEED_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(4, editable.uint8); });
      #if ENABLED(EXTRA_FAN_SPEED)
        EDIT_ITEM_FAST_N(percent, 5, MSG_EXTRA_FAN_SPEED_N, &thermalManager.new_fan_speed[4], 3, 255);
      #endif
    #elif ENABLED(SINGLENOZZLE) && EXTRUDERS > 4
      editable.uint8 = thermalManager.fan_speed[4];
      EDIT_ITEM_FAST_N(percent, 5, MSG_STORED_FAN_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(4, editable.uint8); });
    #endif

    #if HAS_FAN5
      editable.uint8 = thermalManager.fan_speed[5];
      EDIT_ITEM_FAST_N(percent, 6, MSG_FAN_SPEED_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(5, editable.uint8); });
      #if ENABLED(EXTRA_FAN_SPEED)
        EDIT_ITEM_FAST_N(percent, 6, MSG_EXTRA_FAN_SPEED_N, &thermalManager.new_fan_speed[5], 3, 255);
      #endif
    #elif ENABLED(SINGLENOZZLE) && EXTRUDERS > 5
      editable.uint8 = thermalManager.fan_speed[5];
      EDIT_ITEM_FAST_N(percent, 6, MSG_STORED_FAN_N, &editable.uint8, 0, 255, []{ thermalManager.set_fan_speed(5, editable.uint8); });
    #endif
  #endif // FAN_COUNT > 0
